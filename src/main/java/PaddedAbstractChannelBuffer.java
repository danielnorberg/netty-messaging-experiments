...
/**
 * A skeletal implementation of a buffer with padding to avoid contention.
 */
public abstract class PaddedAbstractChannelBuffer implements ChannelBuffer {
  ...
  private String decodeString(final ByteBuffer src, final Charset charset) {
  ...
  protected void checkReadableBytes(int minimumReadableBytes) {
    if (readableBytes() < minimumReadableBytes) {
      throw new IndexOutOfBoundsException("Not enough readable bytes - Need "
                                          + minimumReadableBytes + ", maximum is "
                                          + readableBytes());
    }
  }
}
